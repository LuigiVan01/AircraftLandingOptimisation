function [zdot] = ground2(t, z, u, d, th)
...
A_eq = [  2    1;
        -2*Lr Lf];
b_eq = [3*z_bar + sin(theta_bar)*(Lf - 2*Lr) + M_ver*g/k;
        z_bar*(-Lf + 2*Lr) - sin(theta_bar)*(Lf^2 + 2*Lr^2)];
...
Fs_r        = 2*k*(Z-Lr*sin(theta)-Dr)+2*Fa_r...
                  +2*c*(Z_dot-Lr*cos(theta)*theta_dot); % rear suspensions total force
Fs_fr       = k*(Z+tr*Lf*sin(theta)-Df)+Fa_f...

...
myoptions.GN_funF       = @(X)Ground_cost_GN_new(X,n_free,nu,d,Ts,Tend_td,Tend_b,ds_u_touchdown,ds_u_brake,Q,R,Qf_gr,Qf_dot_gr,x_ref,th,final_state_flight);

%% Initial guess
 load('initialguess_gr.mat');
X0 = Xstar;
X0(1,1)=final_state_flight(2,1);
X0(2,1)=final_state_flight(4,1);
X0(3,1)=final_state_flight(5,1);
X0(4,1)=final_state_flight(6,1);
% X0 = zeros(n_free+nu*N,1);

%% Running the optimization routine
tic
 [Xstar,fxstar] = myfminunc(@(X)Ground_cost_new(X,n_free,nu,d,Ts,Tend_td,Tend_b,ds_u_touchdown,ds_u_brake,Q,R,Qf_gr,Qf_dot_gr,x_ref,th,final_state_flight),X0,myoptions);
...
[f2,z_sim2]=Ground_cost_new(Xstar,n_free,nu,d,Ts,Tend_td,Tend_b,ds_u_touchdown,ds_u_brake,Q,R,Qf_gr,Qf_dot_gr,x_ref,th,final_state_flight);
f2

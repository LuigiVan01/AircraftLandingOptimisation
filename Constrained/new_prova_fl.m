function [zsim, zd, T, L, D, theta] = new_prova_fl(X,z0,nu,nz,d,Ts,Tend,ds_u,Q,R,z_ref,th)
...
index = ones(nu,1);
 ...
 for ind = 2:N+1
     ...
     if zsim((ind-2)*nz+3,1) < 30
         T(index(1,1):end,1) = 0.3;
         L(index(2,1):end,1) = 0.1;
     end
     
     if zsim((ind-2)*nz+3,1) < 10
         T(index(1,1):end,1) = 0.2;
         L(index(2,1):end,1) = 0.6;
         theta(index(4,1):end,1) = 3/20;
     end
     
     if zsim((ind-2)*nz+3,1) < 2
         T(index(1,1):end,1) = 0.1;
         L(index(2,1):end,1) = 1;
         theta(index(4,1):end,1) = 0/20;
     end

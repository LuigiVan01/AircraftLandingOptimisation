...
%
%   INPUTS:
%           fun         =   function providing the scalara cost function value 
...
if strcmp(myoptions.Hessmethod,'GN')    % Constrained Gauss-Newton method
    ...
    % Compute gradient of the Lagrange function
    ...
    while  k < myoptions.nitermax && (norm(gradLagr) > myoptions.tolgrad...
        ...
        [xkp1,T1kp1,niter_LS,tk]    =  	linesearch_merit(T1fun,T1k,DT1k,xk,pk,...
        ...
        deltaf_rel       	=   abs(T1kp1-T1k)/max(eps,abs(T1k));
...
elseif max(eq_constr_max,-ineq_constr_min) > myoptions.tolconstr
    if k >= myoptions.nitermax
        ...
        exitflag    =   -3;
        if strcmp(myoptions.display,'Iter')
            fprintf('Converged to unfeasible point. Constraints not satisfied.\r')
        end
    end
end
